...
// ── Category → icon mapping for cards ────────────────────────────
...
// ── Hardcoded seed posts (shown when Supabase has no published posts yet) ──
// These mirror the original hardcoded array so the page never looks empty.
const SEED_POSTS = [
  { id: 1,  title: "Why the Philippines is the New Epicenter for AI Engineering",      excerpt: "Unpacking the talent shift as Manila transforms from a support hub into a global node for agentic AI development.",           category: "Engineering",    date: "Oct 12, 2024", read_time: "6 min read",  img: IMG_BLOG_1,    author: "Sarah Chen",    featured: true,  status: 'Published' },
  { id: 2,  title: "Scaling to 500+ Agents: A Case Study in Fintech Support",          excerpt: "How a high-growth fintech reduced OpEx by 65% while improving CSAT using managed offshore intelligence.",                     category: "Case Studies",   date: "Sep 28, 2024", read_time: "8 min read",  img: IMG_BLOG_2,    author: "Michael Ross",  featured: false, status: 'Published' },
  { id: 3,  title: "Prompt Engineering vs. Software Engineering: The Merging Path",    excerpt: "Why the developers of 2025 will spend as much time talking to code as they do writing it.",                                   category: "AI Operations",  date: "Sep 15, 2024", read_time: "5 min read",  img: IMG_BLOG_3,    author: "Devin Zhao",    featured: false, status: 'Published' },
  { id: 4,  title: "The Future of CX: Moving Beyond Ticketing to Real-time Agency",   excerpt: "Traditional support is dead. Agentic AI is allowing pods to resolve issues before the user even knows they exist.",            category: "AI Operations",  date: "Aug 30, 2024", read_time: "7 min read",  img: IMG_BLOG_4,    author: "Sarah Chen",    featured: false, status: 'Published' },
  { id: 5,  title: "How Global Logistics Firms are Scaling with Managed Pods",        excerpt: "Exploring the operational blueprint for managing high-volume data and tracking across multiple timezones.",                     category: "Case Studies",   date: "Aug 12, 2024", read_time: "10 min read", img: IMG_BLOG_5,    author: "Marcus Jordon", featured: false, status: 'Published' },
  { id: 6,  title: "Building the Modern Offshore Strategy for Mid-Market Firms",      excerpt: "You don't need enterprise budgets to build enterprise-scale teams. A tactical guide to intelligent scaling.",                  category: "Future of Work", date: "Jul 25, 2024", read_time: "9 min read",  img: IMG_BLOG_6,    author: "Sarah Chen",    featured: false, status: 'Published' },
  { id: 7,  title: "Cybersecurity in the AGI Era: Vetting Your Offshore Partner",     excerpt: "Why SOC-2 compliance and industrial-grade security are the new baseline for global talent partners.",                          category: "Engineering",    date: "Jul 10, 2024", read_time: "6 min read",  img: IMG_ECOM_HERO, author: "Michael Ross",  featured: false, status: 'Published' },
  { id: 8,  title: "PropTech Revolution: AI-Managed Leasing Desks in Manila",         excerpt: "How real estate giants are using specialized offshore teams to handle the entire tenant lifecycle.",                            category: "AI Operations",  date: "Jun 22, 2024", read_time: "5 min read",  img: IMG_PROP_HERO, author: "Devin Zhao",    featured: false, status: 'Published' },
  { id: 9,  title: "Measuring ROI in AI-Augmented Operations: The Hard Truth",        excerpt: "Moving beyond 'headcount cost' to 'outcome velocity'. How to build a modern ROI model for your team.",                        category: "Future of Work", date: "Jun 05, 2024", read_time: "12 min read", img: IMG_CX_HERO,   author: "Marcus Jordon", featured: false, status: 'Published' },
] as Partial<BlogPost>[];

export const BlogLandingPage = ({ setView }: { setView: (v: ViewType) => void }) => {
  const [activeCategory, setActiveCategory] = useState("All");
  const [searchQuery, setSearchQuery]       = useState("");
  const [allPosts, setAllPosts]             = useState<Partial<BlogPost>[]>(SEED_POSTS);
  const [loading, setLoading]               = useState(true);

  // ── Fetch live published posts from Supabase on mount ──────────
  useEffect(() => {
    getBlogPosts()
      .then(data => {
        // If Supabase returns published posts, use them.
        // Otherwise fall back to the seed array above.
        if (data && data.length > 0) {
          setAllPosts(data);
        }
        // If empty, SEED_POSTS stays as the default (set during useState init)
      })
      .catch(() => {
        // Network error — keep showing seed posts silently
      })
      .finally(() => setLoading(false));
  }, []);

  // ── Build dynamic category list from whatever posts are loaded ──
  const categories = [
    "All",
    ...Array.from(new Set(allPosts.map(p => p.category ?? ''))).filter(Boolean).sort(),
  ];

  // ── Featured = first post with featured:true, else first post ──
  ...
  // ── Resolve image — Supabase posts use img or image_url field ──
  const resolveImg = (post: Partial<BlogPost>): string =>
    post.img || post.image_url || '';

  // ── Resolve read time — Supabase uses read_time, seed uses read_time too ──
  const resolveReadTime = (post: Partial<BlogPost>): string =>
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="py-32 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          {/* Loading spinner — only shown briefly on first load */}
          {loading && (
            <div className="flex justify-center py-10">
              <div className="w-7 h-7 border-2 border-[#E61739] border-t-transparent rounded-full animate-spin" />
            </div>
          )}
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center gap-12">
                  <div className="md:w-2/3">
                    ...
                    <div className="flex flex-wrap gap-6 mb-8 border-t border-white/10 pt-8">
                      ...
                      <div className="flex items-center gap-3 text-white/50 text-xs font-bold uppercase tracking-widest">
                        <Clock size={14} /> {resolveReadTime(featuredPost)}
                      </div>
                      <div className="flex items-center gap-3 text-white/50 text-xs font-bold uppercase tracking-widest">
                  ...
                  <div className="md:w-1/3 flex justify-center">
                    <div className="w-64 h-64 rounded-[2.5rem] overflow-hidden border border-white/10 group-hover:scale-105 transition-transform duration-700 shadow-2xl relative">
                      {resolveImg(featuredPost)
                        ? <img src={resolveImg(featuredPost)} alt={featuredPost.title} className="w-full h-full object-cover" />
                  ...
                  <div key={post.id} onClick={() => setView('blog-detail')}
                    ...
                    <div className="flex justify-between items-start mb-8">
                      ...
                      {resolveImg(post) && (
                        <div className="w-16 h-16 rounded-2xl overflow-hidden shrink-0">
                          <img src={resolveImg(post)} alt="" className="w-full h-full object-cover" />
                        </div>
                      )}
                    </div>
                    <h3 className="text-2xl font-bold text-slate-900 mb-4 group-hover:text-[#E61739] transition-colors leading-snug">
                      {post.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between pt-8 border-t border-black/5 mt-auto">
                      <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        <Clock size={12} /> {resolveReadTime(post)}
                      </div>
