...
import { ArrowRight, Plane, Calendar, Users, Globe, ShieldCheck, Map, Hotel, ConciergeBell, LifeBuoy, CreditCard, Compass, Sun, BrainCircuit, CheckCircle2, Ticket, Star, Home } from 'lucide-react';
import { ViewType } from '../../types';
import { Breadcrumbs } from '../../components/Shared';
import { IMG_TRAVEL_HERO as HERO_IMG, IMG_TRAVEL_TEAM as TEAM_IMG } from '../../data';

export const TravelPage = ({ setView }: { setView: (v: ViewType) => void }) => {
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className={`relative bg-[#020202] overflow-hidden pt-48 pb-32 md:pb-40`}>
        <div className="absolute inset-0 z-0">
          <img loading="lazy" src={HERO_IMG} alt="KDCI AI-managed travel and hospitality operations support" className="w-full h-full object-cover opacity-20 object-center" />
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          <Breadcrumbs setView={setView} currentName="Travel & Hospitality" parent={{ name: 'Solutions', view: 'solutions-hub' }} />
          <h1 className="text-5xl md:text-8xl lg:text-9xl font-heading font-bold text-white mb-6 md:mb-10 tracking-tight leading-[0.95] drop-shadow-2xl">
            <span className="text-shine-white">24/7 Guest</span><br/>
            <span className="text-[#E61739]">Experience.</span>
          </h1>
          <p className="max-w-3xl mx-auto text-lg md:text-2xl text-white/60 font-medium leading-relaxed mb-10 md:mb-16 px-4">
            Premium guest support and booking management for travel tech and hospitality groups. Multilingual and always on.
          </p>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-white/5 border-t border-white/10 backdrop-blur-md py-8">
          <div className="max-w-7xl mx-auto px-6 flex flex-wrap justify-center gap-x-12 gap-y-6 md:gap-x-20 lg:gap-x-28 items-center text-white">
            <div className="text-center md:text-left">
              <div className="text-xl md:text-2xl font-black mb-1 text-center">Multilingual</div>
              <p className="text-white/40 text-[9px] md:text-[10px] font-black uppercase tracking-widest">Support</p>
            </div>
            <div className="text-center md:text-left">
              <div className="text-xl md:text-2xl font-black mb-1 text-center">&lt;1 Min</div>
              <p className="text-white/40 text-[9px] md:text-[10px] font-black uppercase tracking-widest">Response Time</p>
            </div>
            <div className="text-center md:text-left">
              <div className="text-xl md:text-2xl font-black mb-1 text-center">High Vol</div>
              <p className="text-white/40 text-[9px] md:text-[10px] font-black uppercase tracking-widest">Booking Mgmt</p>
            </div>
            <div className="text-center md:text-left">
              <div className="text-xl md:text-2xl font-black mb-1 text-center">70</div>
              <p className="text-white/40 text-[9px] md:text-[10px] font-black uppercase tracking-widest">NPS Score</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-32 bg-white relative">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-24">
            <div className="text-[#E61739] text-[10px] font-black uppercase tracking-[0.2em] mb-4">Hospitality Services</div>
            <h2 className="text-4xl md:text-6xl font-heading font-bold text-slate-900 mb-6">What We Do.</h2>
            <p className="text-slate-500 text-xl max-w-3xl mx-auto font-medium">Delivering exceptional guest experiences from booking to checkout.</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: Calendar, title: "Reservation Mgmt", desc: "Handling booking changes, cancellations, and upgrades with ease." },
              { icon: Users, title: "Guest Concierge", desc: "Virtual concierge services for itinerary planning and special requests." },
              { icon: Globe, title: "OTA Management", desc: "Managing listings and availability across Booking.com, Airbnb, and Expedia." },
              { icon: Plane, title: "Flight Rebooking", desc: "Assisting travelers with flight changes and disruptions efficiently." },
              { icon: Star, title: "Loyalty Support", desc: "Managing rewards programs and VIP guest inquiries." },
              { icon: ShieldCheck, title: "Dispute Resolution", desc: "Handling chargebacks and refund negotiations fairly." }
            ].map((s, i) => (
          ...
          <div className="mt-12 p-10 rounded-[3rem] bg-slate-900 text-white flex flex-col md:flex-row items-center gap-8 shadow-2xl relative overflow-hidden">
             ...
             <div className="relative z-10">
               <h4 className="text-2xl font-bold mb-2">High-Touch Service</h4>
               <p className="text-white/60 font-medium">Empathy-driven support for high-stress travel situations, ensuring guests feel heard and helped.</p>
             </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-slate-50 border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
             {[
               { icon: Globe, name: "OTAs" },
               { icon: Hotel, name: "Hotel Chains" },
               { icon: Plane, name: "Airlines" },
               { icon: Home, name: "Vacation Rentals" },
               { icon: Compass, name: "Corporate Travel" }
             ].map((type, i) => (
      ...
      <section className="py-32 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-24">
            <h2 className="text-4xl md:text-6xl font-heading font-bold text-slate-900 mb-6">Service Deployment.</h2>
            <p className="text-slate-500 text-xl font-medium">Getting your travel support team up and running.</p>
          </div>
          <div className="grid md:grid-cols-4 gap-12 relative">
            <div className="hidden md:block absolute top-[2.75rem] left-[10%] right-[10%] h-[1px] bg-slate-100"></div>
            {[
              { step: '01', title: 'GDS Access', desc: 'Connecting to Amadeus, Sabre, or your booking engine.', icon: Map },
              { step: '02', title: 'Brand Training', desc: 'Immersing agents in your service standards and policies.', icon: Users },
              { step: '03', title: '24/7 Setup', desc: 'Establishing shift rotations for round-the-clock coverage.', icon: Calendar },
              { step: '04', title: 'Surge Protocol', desc: 'Preparing for peak travel seasons and disruptions.', icon: Sun }
            ].map((s, idx) => (
         ...
         <div className="max-w-7xl mx-auto px-6 relative z-10">
            <div className="grid lg:grid-cols-2 gap-20 items-center">
               <div>
                  ...
                  <h2 className="text-4xl md:text-6xl font-heading font-bold mb-8 leading-tight">Travel <br/><span className="text-[#E61739]">Intelligence.</span></h2>
                  <p className="text-xl text-white/40 mb-12 leading-relaxed font-medium">
                    AI assists our agents by suggesting itineraries, automating simple rebookings, and triaging urgent requests.
                  </p>
                  
                  <div className="space-y-8">
                     {[
                       { title: "Auto-Rebooking", desc: "AI suggestions for flight alternatives during disruptions." },
                       { title: "Itinerary Gen", desc: "Generating personalized travel plans based on preferences." },
                       { title: "Chatbot Triage", desc: "Handling FAQs about baggage, check-in, and amenities." },
                       { title: "Sentiment Alert", desc: "Flagging frustrated guests for immediate human intervention." }
                     ].map((feat, idx) => (
               ...
               <div className="relative">
                  ...
                  <div className="absolute -bottom-10 -right-10 bg-[#ad1457] p-8 rounded-[3rem] shadow-2xl hidden md:block">
                     ...
                     <div className="text-[10px] font-black uppercase tracking-widest opacity-80">Rebooking Assist</div>
                  </div>
               </div>
            </div>
         </div>
      ...
      <section className="py-32 bg-[#F5F5F7] rounded-t-[5rem]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-6xl font-heading font-bold text-slate-900 mb-6">Flexible Travel Plans.</h2>
            <p className="text-slate-500 text-xl font-medium max-w-3xl mx-auto mb-10">Scaling your hospitality operations without the headcount friction.</p>
          ...
          {pricingModel === 'outcomes' ? (
            <div className="grid md:grid-cols-3 gap-6 items-start animate-in fade-in slide-in-from-bottom-4 duration-500">
              {[
                { 
                  name: "Booking Desk", 
                  price: "$2,200", 
                  desc: "Dedicated reservation management.", 
                  features: ["Itinerary Creation", "Flight/Hotel Changes", "GDS Management", "24/7 Cover Options"], 
                  highlight: false 
                },
                { 
                  name: "Disruption Mgmt", 
                  price: "Custom", 
                  desc: "Emergency rebooking unit.", 
                  features: ["Surge Response Team", "Priority Rebooking", "Refund Negotiation", "Crisis Comms"], 
                  highlight: true, 
                ...
                { 
                  ...
                  price: "$2,000", 
                  desc: "VIP travel assistance.", 
                  features: ["Special Requests", "Local Recommendations", "Loyalty Program Mgmt", "Post-Trip Follow-up"], 
                  highlight: false 
                }
              ].map((plan, i) => (
                <div key={i} className={`p-10 rounded-[3.5rem] relative transition-all ${plan.highlight ? 'bg-slate-900 text-white shadow-2xl scale-105 border-0' : 'bg-white border border-slate-100 shadow-sm hover:shadow-xl'}`}>
                  ...
                  <button onClick={() => setView('contact')} className={`w-full py-4 rounded-[2rem] font-bold text-lg transition-all ${plan.highlight ? 'bg-[#ad1457] text-white hover:bg-[#8e1049] shadow-xl' : 'bg-slate-900 text-white hover:bg-black'}`}>
                    {plan.name === "Disruption Mgmt" ? "Request Custom Quote" : "Request Plan"}
                  </button>
                </div>
              ))}
            </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              {[
                { role: "Travel Consultant", price: "$2,100", focus: "Sales", focusIcon: Plane, desc: "Booking flights, hotels, and creating itineraries." },
                { role: "Guest Support", price: "$1,800", focus: "Service", focusIcon: Users, desc: "Handling inquiries, complaints, and requests." },
                { role: "OTA Manager", price: "$2,000", focus: "Distribution", focusIcon: Globe, desc: "Managing listings and inventory on OTAs." },
                { role: "Ticketing Agent", price: "$1,900", focus: "Ops", focusIcon: Ticket, desc: "Issuing tickets and handling reissues/refunds." }
              ].map((plan, i) => (
          ...
          <div className="mt-12 p-8 bg-white/60 backdrop-blur-md rounded-[3rem] border border-dashed border-slate-300 flex flex-col md:flex-row items-center justify-between gap-8">
             <div className="flex gap-6 items-center">
                <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-[#E61739] shadow-sm"><ConciergeBell size={24} /></div>
                <div>
                   <h4 className="text-lg font-bold text-slate-900">VIP Concierge Team?</h4>
                   <p className="text-sm text-slate-500 font-medium">Dedicated pods for high-net-worth clients and luxury travel.</p>
                </div>
             </div>
      ...
      <section className="py-32 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12">
            {[
              { icon: LifeBuoy, title: "High Touch", desc: "Empathy-driven support for high-stress travel situations." },
              { icon: Map, title: "GDS Systems", desc: "Familiarity with Amadeus, Sabre, and modern booking engines." },
              { icon: Globe, title: "Global Coverage", desc: "Support in multiple languages and time zones." },
              { icon: Sun, title: "Crisis Ready", desc: "Prepared for travel disruptions and surge events." }
            ].map((item, i) => (
      ...
      <section className="py-24 px-6">
        <div className="max-w-7xl mx-auto bg-[#020202] rounded-[5rem] overflow-hidden relative border border-white/5 px-6 py-20 md:p-24 text-center group">
           <div className="absolute inset-0 z-0">
              <img loading="lazy" src={TEAM_IMG} alt="KDCI travel support team delivering AI-managed booking and customer experience operations" className="w-full h-full object-cover opacity-10 group-hover:opacity-20 transition-opacity duration-700" />
           </div>
           ...
           <div className="relative z-10 max-w-4xl mx-auto">
              <h2 className="text-4xl md:text-7xl font-heading font-bold text-white mb-8 tracking-tight leading-tight">Elevate Your <br/><span className="text-shine-red">Guest Experience.</span></h2>
              <p className="text-xl md:text-2xl text-white/60 mb-12 font-medium leading-relaxed">Book a consultation with our travel operations specialists.</p>
              <button onClick={() => setView('contact')} className="px-14 py-6 bg-[#ad1457] text-white rounded-[2rem] font-bold text-xl hover:bg-[#8e1049] transition-all glow-red shadow-2xl flex items-center justify-center gap-4 group mx-auto">
                 Request Travel Quote <ArrowRight size={24} className="group-hover:translate-x-1 transition-transform" />
              </button>
           </div>
        </div>
      </section>
