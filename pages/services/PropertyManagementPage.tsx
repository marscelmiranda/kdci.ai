        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {pricingModel === 'outcomes' ? (
            <div className="grid md:grid-cols-3 gap-6 items-start animate-in fade-in slide-in-from-bottom-4 duration-500">
              {[
                { 
                  name: "Dedicated Associate", 
                  price: "$1,900", 
                  desc: "A dedicated property management assistant.", 
                  features: ["Tenant communication", "Lease administration", "Maintenance coordination", "Direct report to your PM"], 
                  highlight: false 
                },
                { 
                  name: "Managed Operations", 
                  price: "Custom", 
                  desc: "A fully managed team unit for scale.", 
                  features: ["Team Lead oversight", "24/7 coverage options", "AI-driven lead response", "KPI reporting & QA"], 
                  highlight: true, 
                  badge: "Scale Ready" 
                },
                { 
                  ...
                  price: "$1,500", 
                  desc: "Automation layer for inquiry handling.", 
                  features: ["24/7 inquiry response", "Tour scheduling bots", "Pre-qualification logic", "CRM synchronization"], 
                  highlight: false 
                }
              ].map((plan, i) => (
                <div key={i} className={`p-10 rounded-[3.5rem] relative transition-all ${plan.highlight ? 'bg-slate-900 text-white shadow-2xl scale-105 border-0' : 'bg-white border border-slate-100 shadow-sm hover:shadow-xl'}`}>
                  ...
                  <button onClick={() => setView('contact')} className={`w-full py-4 rounded-[2rem] font-bold text-lg transition-all ${plan.highlight ? 'bg-[#E61739] text-white hover:bg-[#c51431] shadow-xl' : 'bg-slate-900 text-white hover:bg-black'}`}>
                    {plan.name === "Managed Ops Pod" ? "Request Pod Quote" : "Request Plan"}
                  </button>
                </div>
              ))}
            </div>
