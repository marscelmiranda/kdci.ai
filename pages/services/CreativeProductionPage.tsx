            ...
            <div className="grid md:grid-cols-3 gap-6 items-start animate-in fade-in slide-in-from-bottom-4 duration-500">
              {[
                { 
                  ...
                  price: "$2,200", 
                  desc: "Unlimited graphic design requests.", 
                  features: ["Integrated into your Figma/Slack", "Follows your brand guides", "Unlimited task queue", "AI workflow support included"], 
                  highlight: false 
                },
                { 
                  name: "Creative Studio", 
                  price: "Custom", 
                  desc: "Multi-disciplinary team + PM.", 
                  features: ["Task execution at massive scale", "Dedicated PM oversight", "Strategic brand consulting", "Full performance analytics"], 
                  highlight: true, 
                  badge: "Enterprise Scale" 
                },
                { 
                  name: "Ad Performance Pack", 
                  price: "$2,800", 
                  desc: "High-velocity creative testing.", 
                  features: ["Auto-versioning & resizing", "Creative QA automation", "Smart tagging systems", "Supports internal designers"], 
                  highlight: false 
                }
              ].map((plan, i) => (
                <div key={i} className={`p-10 rounded-[3.5rem] relative transition-all ${plan.highlight ? 'bg-slate-900 text-white shadow-2xl scale-105 border-0' : 'bg-white border border-slate-100 shadow-sm hover:shadow-xl'}`}>
                  ...
                  <button onClick={() => setView('contact')} className={`w-full py-4 rounded-[2rem] font-bold text-lg transition-all ${plan.highlight ? 'bg-[#E61739] text-white hover:bg-[#c51431] shadow-xl' : 'bg-slate-900 text-white hover:bg-black'}`}>
                    {plan.name === "Creative Studio Pod" ? "Request Custom Quote" : "Request Plan"}
                  </button>
                </div>
              ))}
            </div>
