  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="py-32 bg-[#F5F5F7] rounded-t-[5rem]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-6xl font-heading font-bold text-slate-900 mb-6">Flexible Creative Plans.</h2>
            ...
            <div className="inline-flex bg-white p-1.5 rounded-full border border-slate-200 shadow-sm mb-12">
              <button
                ...
                className={`px-8 py-3 rounded-full text-sm font-black uppercase tracking-widest transition-all ${pricingModel === 'outcomes' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400 hover:text-slate-900'}`}
              >Outcomes</button>
              <button
                onClick={() => setPricingModel('staff-aug')}
                className={`px-8 py-3 rounded-full text-sm font-black uppercase tracking-widest transition-all ${pricingModel === 'staff-aug' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400 hover:text-slate-900'}`}
          ...
          {pricingModel === 'outcomes' ? (
            <div className="grid md:grid-cols-3 gap-6 items-start animate-in fade-in slide-in-from-bottom-4 duration-500">
              {[
                { name: "Design Retainer", price: "$2,200", desc: "Unlimited graphic design requests.", features: ["Integrated into your Figma/Slack", "Follows your brand guides", "Unlimited task queue", "AI workflow support included"], highlight: false },
                { name: "Creative Studio", price: "Custom", desc: "Multi-disciplinary team + PM.", features: ["Task execution at massive scale", "Dedicated PM oversight", "Strategic brand consulting", "Full performance analytics"], highlight: true, badge: "Enterprise Scale" },
                { name: "Ad Performance Pack", price: "$2,800", desc: "High-velocity creative testing.", features: ["Auto-versioning & resizing", "Creative QA automation", "Smart tagging systems", "Supports internal designers"], highlight: false },
              ].map((plan, i) => (
