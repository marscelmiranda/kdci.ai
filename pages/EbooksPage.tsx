...
import { getEbooks, resolveIcon, type Ebook } from '../contentStore';
...
export const EbooksPage = ({ setView }: { setView: (v: ViewType) => void }) => {
  const [papers, setPapers] = useState<Ebook[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getEbooks().then(data => setPapers(data)).finally(() => setLoading(false));
  }, []);
      ...
      <section className="relative pt-48 pb-32 overflow-hidden bg-[#020202]">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          ...
          <h1 className="text-5xl md:text-8xl font-heading font-bold text-white mb-8 tracking-tight leading-[1] drop-shadow-2xl">
            <span className="text-shine-white">Strategic</span><br/><span className="text-[#E61739]">Deep Dives.</span>
          </h1>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {!loading && (
            <div className="grid md:grid-cols-2 gap-12">
              {papers.map((paper) => {
                const BgIcon = resolveIcon(paper.icon_name);
                return (
                  <div key={paper.id} className="group relative bg-[#F5F5F7] rounded-[3rem] p-10 overflow-hidden border border-black/[0.03] hover:shadow-2xl transition-all duration-500">
                    <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity transform group-hover:scale-110 duration-700">
                      <BgIcon size={200} className="text-slate-900" />
                    </div>
                    <div className="relative z-10 flex flex-col h-full">
                      ...
                      <p className="text-slate-500 font-medium leading-relaxed mb-10 max-w-md">{paper.description}</p>
                      {paper.pdf_url ? (
                        <a href={paper.pdf_url} target="_blank" rel="noopener noreferrer"
                          className="mt-auto self-start px-8 py-4 bg-white border border-slate-200 rounded-2xl font-bold text-sm hover:bg-slate-900 hover:text-white hover:border-transparent transition-all flex items-center gap-3">
